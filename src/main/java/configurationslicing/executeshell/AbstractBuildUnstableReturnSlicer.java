...
public abstract class AbstractBuildUnstableReturnSlicer<B extends Builder> extends UnorderedStringSlicer<AbstractProject> {
    ...
    public abstract static class AbstractBuildUnstableReturnSliceSpec<B extends Builder>
        ...
        public List<String> getValues(AbstractProject item) {
            ...
            for (B builder : builders) {
                content.add(getUnstableReturn(builder));
            }
            if (content.isEmpty()) {
                content.add(NOTHING);
            }
        ...
        public boolean setValues(AbstractProject item, List<String> list) {
            ...
            for (int i = 0; i < list.size(); i++) {
                if (oldBuilders[i] != null) {
                    newBuilders[i] = createBuilder(list.get(i), builders, oldBuilders[i]);
                }
            }
            ...
            // add any new ones (should always add to the end, but might not if the original UnstableReturn was empty)
            for (int i = 0; i < maxLen; i++) {
            ...
            for (int i = 0; i < maxLen; i++) {
                if (oldBuilders[i] != null && newBuilders[i] == null) {
                    // the remove UnstableReturn will persist the project
